...
void DiMuonSystemConsumer::Init(setting_type const& settings)
{
	ConsumerBase<KappaExampleTypes>::Init(settings);
...
void DiMuonSystemConsumer::Finish(setting_type const& settings)
{
	// save histogram
	RootFileHelper::SafeCd(settings.GetRootOutFile(),
	                       settings.GetRootFileFolder());

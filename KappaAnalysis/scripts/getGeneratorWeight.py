...
def main():
	...
	parser.add_argument("files", nargs="+", help="Input filelists of the collection")
	...
	for samplelist in args.files:
		with open(samplelist) as samplelist:
			samplefiles = samplelist.readlines()

	for filelist in samplefiles:
		filelist=filelist.strip()
		filelist =filelist.split("/",2)[-1]
		with open(filelist) as lists:
			rootfiles = lists.readlines()

		sumweight_per_nick = {}
		n_entries_per_nick = {}
		for (fileindex, file_name) in enumerate(rootfiles):

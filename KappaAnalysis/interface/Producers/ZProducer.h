...
{
public:
	ZProducerBase(std::vector<TLepton1*> product_type::*validLeptons1,
	              std::vector<TLepton2*> product_type::*validLeptons2,
	...
	void Init(setting_type const& settings, metadata_type& metadata) override
	{
		...
		LambdaNtupleConsumer<KappaTypes>::AddFloatQuantity("ZMass", [](event_type const & event, product_type const & product)
		{
			return product.m_z.p4.M();
		});

		LambdaNtupleConsumer<KappaTypes>::AddFloatQuantity("leadingLeptonFromZPt", [](event_type const & event, product_type const & product)
		{
			return product.m_zValid ? product.m_zLeptons.first->p4.Pt() : DefaultValues::UndefinedFloat;
		});

		LambdaNtupleConsumer<KappaTypes>::AddFloatQuantity("leadingLeptonFromZEta", [](event_type const & event, product_type const & product)
		{
			return product.m_zValid ? product.m_zLeptons.first->p4.Eta() : DefaultValues::UndefinedFloat;
		});

		LambdaNtupleConsumer<KappaTypes>::AddFloatQuantity("leadingLeptonFromZPhi", [](event_type const & event, product_type const & product)
		{
			return product.m_zValid ? product.m_zLeptons.first->p4.Phi() : DefaultValues::UndefinedFloat;
		});

		LambdaNtupleConsumer<KappaTypes>::AddFloatQuantity("trailingLeptonFromZPt", [](event_type const & event, product_type const & product)
		{
			return product.m_zValid ? product.m_zLeptons.second->p4.Pt() : DefaultValues::UndefinedFloat;
		});

		LambdaNtupleConsumer<KappaTypes>::AddFloatQuantity("trailingLeptonFromZEta", [](event_type const & event, product_type const & product)
		{
			return product.m_zValid ? product.m_zLeptons.second->p4.Eta() : DefaultValues::UndefinedFloat;
		});

		LambdaNtupleConsumer<KappaTypes>::AddFloatQuantity("trailingLeptonFromZPhi", [](event_type const & event, product_type const & product)
		{
			return product.m_zValid ? product.m_zLeptons.second->p4.Phi() : DefaultValues::UndefinedFloat;
		});
		LambdaNtupleConsumer<KappaTypes>::AddFloatQuantity("thetaZLepMinus", [](event_type const & event, product_type const & product)
		{
			return product.m_theta_Z_LepMinus;
		});
		LambdaNtupleConsumer<KappaTypes>::AddBoolQuantity("validZ", [](event_type const & event, product_type const & product)
		{
			return product.m_zValid;
		});
		LambdaNtupleConsumer<KappaTypes>::AddIntQuantity("nZCandidates", [](event_type const & event, product_type const & product)
		{
...
private:
	...
	bool lepton_pair_onZ(KLepton* const lep1, KLepton* const lep2, setting_type const& settings) const
	{
	...
	bool is_closer_to_Z(double zCandidate_mass, product_type& product, setting_type const& settings) const{
		if (!product.m_zValid) return true;
		return (fabs(zCandidate_mass-settings.GetZMass()) < fabs(product.m_z.p4.M()-settings.GetZMass()));
	}
	void resetZ(product_type& product) const{
		product.m_z = KLV();
		product.m_z.p4.SetPt(0.0f);  // just to be sure
		product.m_zLeptons = std::make_pair(nullptr, nullptr);
		product.m_zValid = false;
	}
	void setZ(product_type& product, KLepton* const lep1, KLepton* const lep2, setting_type const& settings) const
	{
		KLV zCandidate;
	...
	const KPFCandidate* determine_pfCandidate_for_lepton(product_type& product, setting_type const& settings,
	...
	double calculate_theta_Z_LepMinus(product_type& product) const
	{

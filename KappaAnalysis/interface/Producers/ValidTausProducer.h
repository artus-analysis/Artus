...
private:
	...
	bool IsTauIDRecommendation13TeV(KTau* tau, KappaEvent const& event) const
	{
		...
		return (( tau->getId("hpsPFTauDiscriminationByDecayModeFinding", event.m_tauMetadata) > 0.5
				|| tau->getId("hpsPFTauDiscriminationByDecayModeFindingNewDMs", event.m_tauMetadata) > 0.5)
				&& (tau->getId("hpsPFTauDiscriminationByMVA5VLooseElectronRejection", event.m_tauMetadata) > 0.5)
				&& (tau->getId("hpsPFTauDiscriminationByLooseMuonRejection3", event.m_tauMetadata) > 0.5)
		...
		//		|| tau->track.getDz(vertex) < -1.5 )
				&& (tau->getDiscriminator("hpsPFTauDiscriminationByRawCombinedIsolationDBSumPtCorr3Hits", event.m_tauMetadata) < 1.0));
	}

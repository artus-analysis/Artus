...
\section{Common Workflow of High Energy Physics Analyses}
...
Still, equal or very similar requirements are also present for HEP experiments not located at the LHC.
...
The reconstruction of the raw data coming from the experiments is performed on large computing centers organized into the WLCG\footnote{\url{http://wlcg-public.web.cern.ch}}, the Worldwide LHC Computing Grid.
The resulting datasets contain event information and high-level physics objects and are of $\mathcal O(10-100)$~TB in size.
\todo[inline]{move file size numbers to Figure \ref{figure_analysis_steps}?}
...
Source code and configuration files have become the most precise form of documentation.
...
This structure extends the established concepts of existing frameworks like FWLite from CMSSW to the following four main advantages:
\begin{description}
...
\section{HarryPlotter -- Python Post-processing Framework}
...
In conjunction with the plotting toolkit HarryPlotter, all data analysis needs are covered.

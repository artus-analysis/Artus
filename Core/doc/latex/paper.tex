...
\caption[Characteristics of an event-based analysis in high energy physics]{Characteristic of an event-based analysis in high energy physics, numbers taken from \cite{joram_phd}. Data size is reduced by six orders of magnitude, turnaround time by 4. I/O intensive operations are done in highly parallel in the WLCG.
...
The main performance indicator is the \emph{turnaround time}, which means the time that is needed for the whole analysis in case new requirements arise. Since the runtime is often limited by the transfer speed to the storage elements, \artus is designed to load each event only once from the storage system and performs all necessary steps directly. This is made possible with the pipelining concept. Still, since analysis requirements sometimes change within a few hours, a high flexibility is needed. This explains the need for a modular structure, where only those parts of the code are executed that really are necessary for the analysis.
\subsection{Maintainability}
Keeping code maintained, i.e. up-to-date with the latest requirements while not losing backwards compatibility is a challenge for each software project. The first important step is an always updated recipe that works out of the box on each machine offering CVMFS. To make the entry level as low as possible, beginners can start with an example analysis. In \artus, everything is steered via JSON config files. The modules then implement what has to be done while the data is stored in desired data format.
...
In most cases, analysis never leave prototype status. This leads to frequent re-implementations and low trust in reliability. \artus allows the re-use of analysis code due to its modular structur as well as sharing it between different analysis groups, with improves code quality by far.

Sourcecode and configuration files have become the most precise form of documentation. Therefore, readability also for people who have not written the code is crucial. Also, if possible, standard existing libraries from HEP are interfaced.

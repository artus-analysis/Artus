...
#		return
	...
	if settings['xynames'][1] == 'events' and 'y' not in opt.user_options:
		ax.set_ylim(top=max(d.ymax() for d in plotsdict["plots"][plotname]["collections"][collection][affiliation]["mplhistos"]) * 1.2)
	...
	if settings['log']:
		if 'y' not in opt.user_options:
			ax.set_ylim(bottom=1.0, top=max(d.ymax() for d in plotsdict["plots"][plotname]["collections"][collection][affiliation]["mplhistos"]) * 2)
		ax.set_yscale('log')

...
struct CrystalBall2016{
    ...
    CrystalBall2016(double m_, double s_, double a_, double n_){
	init(m_, s_, a_, n_);
    }
...
class RocRes2016{
	...
	CrystalBall2016  cb[NMAXETA][NMAXTRK];
...
class RocOne2016{
	...
	RocOne2016(std::string filename, int iTYPE=0, int iSYS=0, int iMEM=0);
	...
	double kGenSmear(double pt, double eta, double v, double u, RocRes2016::TYPE TT=RocRes2016::Data) const;
...
class RoccoR2016{
    public:
	RoccoR2016();
	RoccoR2016(std::string dirname);
	~RoccoR2016();

	void init(std::string dirname);

	double kGenSmear(double pt, double eta, double v, double u, RocRes2016::TYPE TT=RocRes2016::Data, int s=0, int m=0) const;
    ...
    private:
	std::vector<std::vector<RocOne2016> > RC;
};
